...
out_c = "#pragma once\n#include <stdint.h>\n"
out_c += f"const int SLEEK_DIGIT_H = {H};\n"
...
for c in chars:
    ...
    if c == ':':
        ...
        W = int(img_cropped.width * (H / img_cropped.height))
...
out_c += "const uint8_t sleek_digit_widths[11] = {" + ", ".join(map(str, widths)) + "};\n"

offsets = [0]
for idx, b_list in enumerate(bitmaps):
    offsets.append(offsets[-1] + len(b_list))

out_c += "const uint16_t sleek_digit_offsets[11] = {" + ", ".join(map(str, offsets[:-1])) + "};\n"

out_c += "const uint8_t sleek_digits[" + str(offsets[-1]) + "] = {\n  "
lines = []
...
with open("src/SleekFont.h", "w") as f:
    f.write(out_c)
print("SleekFont.h generated")
